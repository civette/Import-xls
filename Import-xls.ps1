...
{   <# 
    ...
    Created: 2011-03-27 
    Modified by Didier DEBELDER: 2019-08-01 
        ...
        {   if ($Force -or $psCmdlet.ShouldProcess($_))
            {   $fileExist = Test-Path $_ 
                ...
                {   # create temporary .csv file from excel file and import .csv 
                    # 
                    $_ = convert-path ((Resolve-Path $_).toString())
                    $wb = $xl.Workbooks.Add($_); 
                    if ($?)
                    {   $csvTemp = GetTempFileName(".csv"); 
                        $ws = $wb.Worksheets.Item($Worksheet); 
                        $ws.SaveAs($csvTemp, $xlFileFormats[".csv"], [Type]::Missing, [Type]::Missing, [Type]::Missing, [Type]::Missing, [Type]::Missing, [Type]::Missing, [Type]::Missing, $true)
                        $wb.Close($false);
